...
function enregistrer(id,nb){
  const produit = document.getElementById(nb+'_produit').value;
  const ht = document.getElementById(nb+'_HT').value;
  const tva = document.getElementById(nb+'_TVA_20p').value;
  const ttc = document.getElementById(nb+'_TTC').value;

  const txt_produit = document.getElementById(nb+'_txt_produit');
  const txt_HT = document.getElementById(nb+'_txt_HT');
  const txt_TVA= document.getElementById(nb+'_txt_TVA_20p');
  const txt_TTC = document.getElementById(nb+'_txt_TTC');

  const request = new XMLHttpRequest();

  request.open("POST","verif/verif_admin_petrole.php");
  ...
  request.send("produit="+produit+"&HT="+ht+"&TVA="+tva+"&TTC="+ttc+"&id="+id);
}
