...
function enregistrer(id,nb){
  const horaires = document.getElementById(nb+'_horaires').value;
  ...
  const txt_horaires = document.getElementById(nb+'_txt_horaires');
  ...
  request.open("POST","verif/verif_admin_red_atterrissage.php");
  ...
  request.onreadystatechange=function(){
    if (request.readyState===4 && request.status===200) {
      if(request.responseText=="ok"){
        ...
        txt_horaires.innerHTML=horaires;
        txt_HT.innerHTML=ht+"€";
  ...
  request.send("horaires="+horaires+"&HT="+ht+"&TVA="+tva+"&TTC="+ttc+"&id="+id);
}
